...
export default function QLToolHome(props: Props) {
    ...
    const createSession = () => {
        ...
        axios.get(ENDPOINT + '/session/create')
            ...
            .then(res => {
                ...
                if (json?.id != null) {
                    sessionData.id = json.id;
                }
                if (json?.connected != null) {
                    sessionData.connected = json.connected;
                }
                if (json?.join_code != null) {
                    sessionData.joinCode = json.join_code;
                }
                if (customId != '') {
                    sessionData.id = customId;
                }
    ...
    return (
        <Box sx={styles.container}>
            ...
            {connectionState == ConnectionState.CONNECTED && <Typography>Join Code: {props.sessionData.joinCode}</Typography>}
            {connectionState == ConnectionState.CONNECTED && <Typography>ID: {props.sessionData.id}</Typography>}
            ...
            {connectionState != ConnectionState.CONNECTED && <Button variant="contained" sx={styles.button} onClick={createSession}>Create Session</Button>}
            {connectionState != ConnectionState.CONNECTED && <Button variant="contained" sx={styles.button} onClick={joinSession}>Join Session</Button>}
        </Box>
    )
}